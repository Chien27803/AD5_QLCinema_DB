...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupClickListeners() {
        ...
        cardQuickBooking.setOnClickListener(v -> {
            ...
            Intent intent = new Intent(MainActivity.this, AllShowtimesActivity.class);
    ...
    private void loadCurrentUserSession() {
        // 1. Cố gắng lấy từ Intent (Nếu vừa đăng nhập)
        currentUser = (User) getIntent().getSerializableExtra("user");

        if (currentUser == null) {
            // 2. Nếu Intent trống, kiểm tra Session (SharedPreferences)
            int userId = SessionManager.getLoggedInUserId(this);

            if (userId > 0) {
                // Nếu ID tồn tại trong Session, tải chi tiết user từ DB
                User sessionUser = dbHelper.getUserById(userId);

                if (sessionUser != null) {
                    currentUser = sessionUser;
                } else {
                    // ID tồn tại nhưng user không tồn tại, tạo Guest
                    createGuestUser();
                }
            } else {
                // 3. Nếu Session ID không tồn tại hoặc <= 0, tạo Guest
                createGuestUser();
            }
        }

        // Cần đảm bảo rằng hàm này được gọi đúng cách
        // if (currentUser != null) { tvUsername.setText(currentUser.getUsername()); }
    }

    // Hàm phụ trợ tạo Guest User
